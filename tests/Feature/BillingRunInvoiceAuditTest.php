...
class BillingRunInvoiceAuditTest extends TestCase
{
    ...
    public function test_billing_runs_list_computes_members_billed_and_total_from_invoices(): void
    {
        NumberSeries::factory()->create(['code' => 'INV', 'prefix' => 'INV-']);
        ...
        // A historical run whose stored counters are stale (invoices were posted
        // by the daily generator, so the run recorded 0).
        $run = ShareBillingRun::create([
            'title' => 'Stale run',
            'billing_schedule_id' => $schedule->id,
            'billing_date' => today(),
        ...
        Livewire::test(ListShareBillingRuns::class)
            ->assertCanSeeTableRecords([$run])
            ->assertTableColumnStateSet('invoices_count', 1, $run)
            ->assertTableColumnStateSet('sales_lines_sum_line_amount', '100000.0000', $run);
    }
}
